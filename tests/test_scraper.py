...
class TestWebScraper:
    ...
    def test_check_website_success(self):
        ...
        # Get config values directly from scraper's config
        timeout = self.scraper.config.fetch_config_value("scraping", "timeout", 10)
    ...
    def test_check_website_request_exception(self):
        ...
        # Get config values directly from scraper's config
        max_retries = self.scraper.config.fetch_config_value("scraping", "max_retries", 3)
    ...
    def test_retry_logic_with_eventual_success(self):
        ...
        # No need to get config values - we just need to test that retry logic works
    ...
    def test_check_website_timeout_exception(self):
        """Test handling of timeout exceptions with retry logic"""
        # Get config values directly from scraper's config
        max_retries = self.scraper.config.fetch_config_value("scraping", "max_retries", 3)
